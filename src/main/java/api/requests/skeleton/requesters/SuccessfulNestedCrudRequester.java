...
import api.requests.skeleton.interfaces.NestedCrudEndpoint;
...
public class SuccessfulNestedCrudRequester<CREATE, UPDATE, RES> {
    ...
    public RES update(String parentId, String id, UPDATE request) {
        return requester.update(parentId, id, request)
                .then()
                //TODO Документация указывает 201, бэк возвращает 200.
                // 200 в тестах считается временным контрактом до уточнения
                .statusCode(SC_OK)
                .extract()
